// @description obj_crystal_core — tilemap defaults from parent + Hollow Knight 6-state crystal FSM.
event_inherited();

// Slash-only contact — never use grounded body touch damage (parent collision still separates)
...
// Downward slash hitboxes — local space from feet origin (+X = facing right), per spr_crystal_core_attack frame.
...
// Combat pose sprites — idle uses spr_crystal_core; telegraph/attack swap body + glow art.
ENEMY_SPRITE_IDLE = spr_crystal_core;
ENEMY_SPRITE_WINDUP = spr_crystal_core_windup;
ENEMY_SPRITE_ATTACK = spr_crystal_core_attack;
