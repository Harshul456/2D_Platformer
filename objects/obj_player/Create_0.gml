...
DASH_DIR_LOCK_FRAMES = 12;      // Initial dash/burst: commit dir, ignore opposite / release
DASH_FRAMES = 12;               // Standstill tap-Z dash length (eased — longer = smoother)
DASH_SPEED = 8.2;               // Peak standstill dash speed
DASH_EASE_IN = 0.30;            // Fraction of dash spent ramping up (smoothstep)
DASH_EASE_OUT = 0.28;           // Fraction of dash spent easing down before reel
DASH_START_MULT = 0.38;         // Speed as fraction of peak on first frame (not a hard snap)
DASH_END_MULT = 0.42;           // Speed as fraction of peak on last frame
...
DASH_BURST_PARTICLE_COUNT = 12; // Crystal-style 1px white motes on dash activate
...
PERFECT_DODGE_COAST_HSP     = 4.2;  // Base through-enemy horizontal (short hop past)
PERFECT_DODGE_COAST_MIN     = 3.2;  // Floor speed while crossing
PERFECT_DODGE_COAST_MAX     = 5.5;  // Cap — don't launch across the room
PERFECT_DODGE_CLEAR_PX      = 20;   // Modest past far edge (stay close for counter)
PERFECT_DODGE_HSP_DECAY     = 0.96; // Bleed speed after the cross so you don't skate away
PERFECT_DODGE_GRAV_MUL      = 0.48; // Softer drop to match the higher hop
PERFECT_DODGE_GHOST_INTERVAL = 2;   // Spawn afterimage every N frames
// Slow-mo lighting (Bulb ambient crush + icy focus — replaces purple vignette)
PERFECT_DODGE_LIGHT_AMB_R   = 3;    // Ambient RGB while fully in slow-mo
PERFECT_DODGE_LIGHT_AMB_G   = 5;
PERFECT_DODGE_LIGHT_AMB_B   = 12;
...
DODGE_COUNTER_LUNGE_HSP     = 6;    // Closing speed during flurry streak
DODGE_COUNTER_DAMAGE_MULT   = 1.75; // Total damage feel vs a normal swing (split across chips)
DODGE_COUNTER_CHIP_MULT     = 0.28; // Damage fraction per flurry tick (barrage)
DODGE_COUNTER_HIT_INTERVAL  = 3;    // Frames between multi-hits (slower barrage)
DODGE_COUNTER_VANISH_FRAMES = 8;    // Fade-out + ghost burst before flurry
DODGE_COUNTER_REAPPEAR_FRAMES = 18; // Soft fade-in while landing crouch plays
DODGE_COUNTER_STRIKE_OFFSET = 28;   // Stand this far from enemy center on reappear
DODGE_COUNTER_FLURRY_FRAME_HOLD = 6;// Game frames per flurry sprite frame (slower FX)
...
perfect_dodge_light_t = 0;          // 0 = normal cave light, 1 = full slow-mo focus
perfect_dodge_focus_light = undefined;
perfect_dodge_distort_fired = false;
perfect_dodge_aura_timer = 0;       // Delay before blue focus circle (after distort finishes)
dodge_counter_strike = false;       // Empowered hit while state == DODGE_COUNTER
dodge_counter_phase = 0;            // DODGE_COUNTER_PHASE
...
dodge_counter_to_y = 0;
dodge_counter_hidden = false;       // Body invisible during streak
